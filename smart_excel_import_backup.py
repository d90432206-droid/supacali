...
class SmartExcelParser:
    ...
    def standardize_dataframe(self, df: pd.DataFrame, detected_cols: dict, excel_path: Path) -> pd.DataFrame:
        """將 DataFrame 標準化為系統格式"""
        # 清理空列
        df = df.dropna(how='all')
        
        # 提取基本資訊
        order_number = self.extract_order_number(df, detected_cols)
        
        # 優先從 B9 提取客戶名稱（Excel 格式：Row 9 = Index 8）
        customer_name = "未知客戶"
        try:
            if len(df) > 8 and len(df.columns) > 1:  # 確保有 B9 儲存格
                b9_value = df.iloc[8, 1]  # Row 9 (Index 8), Column B (Index 1)
                if pd.notna(b9_value) and str(b9_value).strip():
                    customer_name = str(b9_value).strip()
                    print(f"  ✓ 從 B9 提取客戶: {customer_name}")
        except:
            pass
        
        # 如果 B9 沒有客戶，使用原邏輯
        if customer_name == "未知客戶":
            customer_name = self.extract_customer(df, detected_cols)
        ...
        if order_number == "UNKNOWN":
            ...
            if match:
                ...
                order_number = excel_path.stem
        
        print(f"  報價單號: {order_number}")
        print(f"  客戶: {customer_name}")
        
        # 建立標準化資料
        standardized_data = []
        
        # 根據用戶格式：B欄=項目名稱, D欄=數量, E欄=單價
        # 假設從第 10 列開始是明細資料（跳過標題）
        for idx, row in df.iterrows():
            # 跳過前面的標題列（通常前 10 列是表頭）
            if idx < 10:
                continue
            ...
            try:
                ...
                equipment_name = ""
                if len(row) > 0:
                    eq_name_value = row.iloc[0] if hasattr(row, 'iloc') else row[df.columns[0]]
                    if pd.notna(eq_name_value):
                        equipment_name = str(eq_name_value).strip()
                
                # B 欄 (Index 1) = 產品名稱（校正項目）
                product_name = None
                if len(row) > 1:
                    product_name = row.iloc[1] if hasattr(row, 'iloc') else row[df.columns[1]]
                ...
                # 過濾掉明顯不是產品的列（標題、備註等）
                skip_keywords = ['項目', '序號', '品名', '數量', '單價', '合計', '小計', '總計', '備註', '項次']
                if any(kw in product_name_str for kw in skip_keywords):
                    continue
                
                # 如果只是數字或很短，跳過
                if len(product_name_str) < 2 or product_name_str.isdigit():
                    continue
                
                # C 欄 (Index 2) = 設備案號/設備編號
                equipment_number = ""
                if len(row) > 2:
                    eq_num_value = row.iloc[2] if hasattr(row, 'iloc') else row[df.columns[2]]
                    if pd.notna(eq_num_value):
                        equipment_number = str(eq_num_value).strip()
                
                # D 欄 (Index 3) = 數量
                quantity = 1
                has_valid_qty = False
                if len(row) > 3:
                    qty_value = row.iloc[3] if hasattr(row, 'iloc') else row[df.columns[3]]
                ...
                if len(row) > 4:
                    price_value = row.iloc[4] if hasattr(row, 'iloc') else row[df.columns[4]]
                    if pd.notna(price_value):
                        try:
                            unit_price = float(price_value)
                            if unit_price > 0:  # 價格必須 > 0
                                has_valid_price = True
                        except:
                            unit_price = 0
                
                # ⭐ 關鍵過濾：只保留同時有數量和價格的列
                if not (has_valid_qty and has_valid_price):
                    continue
                ...
                item = {
                    '報價單號': order_number,
                    '客戶名稱': customer_name,
                    '產品名稱': product_name_str,
                    '數量': quantity,
                    '單價': unit_price,
                    '規格': '',  # 可從產品名稱中提取
                    '報價日期': '',
                    '項目編號': '',
                    '設備名稱': equipment_name if equipment_name else product_name_str,  # 使用 A 欄設備名稱
                    '設備案號': equipment_number,  # 使用 C 欄設備案號
                    '校正類型': '內校',
        ...
        if len(result_df) > 0:
            ...
            for idx, row in result_df.head(3).iterrows():
                print(f"     - {row['產品名稱']} | 數量:{row['數量']} | 單價:${row['單價']:,.0f}")
