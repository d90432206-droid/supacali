...
export const OrderForm: React.FC<OrderFormProps> = ({ onOrderCreated, copyData }) => {
  ...
  const addToCart = () => {
    ...
    // Optimistic Update: Add to local inventory state if it's a new item to be saved
    if (newItem.saveToInventory && !newItem.productId) {
      // Create a temporary product object
      const tempProduct: Product = {
        id: 'TEMP-' + Date.now(), // Temporary ID for search mapping
        name: newItem.productName,
        specification: newItem.productSpec,
        category: newItem.category,
        standardPrice: newItem.price,
        lastUpdated: new Date().toISOString()
      };

      // Update inventory state so it appears in search immediately
      setInventory(prev => [...prev, tempProduct]);

      // Link cart item to this temp ID so we don't create duplicates if added again
      newItem.productId = tempProduct.id;
    }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Map to store temporary ID -> Real ID mappings to avoid double creation
      const tempIdMap = new Map<string, string>();

      for (const item of cart) {
        let finalProductId = item.productId;

        // Handle New Products (TEMP items or explicitly saved items without ID)
        // Check if it's a TEMP id or (empty ID + saveToInventory)
        const isTempId = finalProductId && finalProductId.startsWith('TEMP-');
        const needsCreation = isTempId || (!finalProductId && item.saveToInventory);

        if (needsCreation) {
          // Check if we already created this product in this loop (deduplication)
          const mapKey = isTempId ? finalProductId : `${item.productName}-${item.productSpec}`;

          if (tempIdMap.has(mapKey)) {
            finalProductId = tempIdMap.get(mapKey)!;
          } else {
            // Actual creation API call
            try {
              ...
              tempIdMap.set(mapKey, finalProductId);

            } catch (e) {
              console.error("Failed to auto-create product:", item.productName);
              // Fallback: leave as is, might fail later or be saved as ad-hoc
            }
          }
            ...
            {availableTechnicians.length === 0 ? (
              ...
              availableTechnicians.map(tech => (
                <button
                  ...
                  className={`px-3 py-1.5 rounded-full text-xs font-medium border transition-colors ${selectedTechnicians.includes(tech.name)
              ...
              {!currentItem.productId && productNameQuery && (
                <div className="pt-2">
                  <button
                    ...
                    className={`w-full py-2 px-3 rounded-md text-xs font-bold transition-all flex items-center justify-center gap-2 border ${currentItem.saveToInventory
