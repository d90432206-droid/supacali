...
// --- Subcomponent: Editable Row ---
const EditableOrderRow: React.FC<{
    item: Order;
    onUpdate: (id: string, updates: { quantity: number; unitPrice: number; totalAmount: number }) => Promise<void>;
    isEditable: boolean;
    ...
    // Calculate discount multiplier (e.g. 80 -> 0.8)
    // Default to 1 (100%) if missing
    const discountMult = (item.discountRate || 100) / 100;

    const handleQtyChange = (newQty: number) => {
        // Change Qty -> Recalc Total (Price constant)
        const newTotal = Math.round(newQty * editData.unitPrice * discountMult);
        setEditData({ ...editData, quantity: newQty, totalAmount: newTotal });
    };

    const handlePriceChange = (newPrice: number) => {
        // Change Price -> Recalc Total (Qty constant)
        const newTotal = Math.round(editData.quantity * newPrice * discountMult);
        setEditData({ ...editData, unitPrice: newPrice, totalAmount: newTotal });
    };

    const handleTotalChange = (newTotal: number) => {
        // Change Total -> Recalc Price (Qty constant)
        // Price = Total / (Qty * Discount)
        let newPrice = 0;
        if (editData.quantity > 0 && discountMult > 0) {
            newPrice = Math.round(newTotal / (editData.quantity * discountMult));
        }
        setEditData({ ...editData, totalAmount: newTotal, unitPrice: newPrice });
    };
            ...
            <tr className="bg-brand-50/30">
                ...
                <td className="px-3 py-2 text-right font-mono">
                    <input
                        ...
                        className="w-20 text-right px-1 py-0.5 border border-brand-300 rounded focus:ring-1 focus:ring-brand-500 text-sm outline-none bg-white"
                    />
                </td>
                <td className="px-3 py-2 text-right">
                    <input
                        ...
                        className="w-16 text-right px-1 py-0.5 border border-brand-300 rounded focus:ring-1 focus:ring-brand-500 text-sm outline-none bg-white"
                    />
                </td>
                <td className="px-3 py-2 text-right font-bold text-slate-700">
                    <input
                        ...
                        className="w-20 text-right px-1 py-0.5 border border-brand-300 rounded focus:ring-1 focus:ring-brand-500 text-sm outline-none font-bold bg-white"
                    />
                </td>
    ...
    return (
        <tr className="hover:bg-slate-50 group/row">
            ...
            <td className="px-3 py-2 text-right font-bold text-slate-700">${item.totalAmount.toLocaleString()}</td>
            <td className="px-3 py-2 text-right">
                {isEditable && (
                    <button
...
export const OrderList: React.FC<OrderListProps> = ({ orders, refreshData, onCopyOrder }) => {
    ...
    const handleItemUpdate = async (id: string, updates: { quantity: number; unitPrice: number; totalAmount: number }) => {
        await mockGasService.updateOrderItem(id, updates);
        refreshData(); // Refresh to update grouped totals
    };
                                            ...
                                            <tr className="bg-slate-50/50 shadow-inner">
                                                <td colSpan={7} className="p-4 pl-12">
                                                    <div className="bg-white rounded border border-slate-200 overflow-hidden">
                                                        <div className="bg-slate-100 px-3 py-1 text-xs font-bold text-slate-500 uppercase flex justify-between items-center">
                                                            <span>校正項目明細</span>
                                                            {viewMode === 'active' && <span className="text-[10px] text-brand-600 normal-case bg-brand-50 px-2 rounded">提示：移動滑鼠至行尾可編輯數量與金額</span>}
                                                        </div>
                ...
                {groupedOrders.map((group) => {
                    ...
                    return (
                        <div key={group.orderNumber} className={`bg-white rounded-lg border border-slate-200 shadow-sm p-4 space-y-3 ${group.isArchived ? 'opacity-80' : ''}`}>
                            ...
                            {/* Mobile Expandable Details (Read Only for now on mobile to save space) */}
                            {isExpanded && (
                                <div className="bg-slate-50 p-2 rounded border border-slate-100 space-y-2">
                                    <div className="text-center text-[10px] text-slate-400">若需編輯明細請使用電腦版</div>
                                    {group.items.map((item, idx) => (
                                        <div key={idx} className="border-b border-slate-100 last:border-0 pb-2 last:pb-0">
                                            ...
                                            <div className="flex justify-between items-end mt-1">
                                                ...
                                                <div className="text-xs font-bold text-slate-600">${item.unitPrice} x {item.quantity} = ${item.totalAmount}</div>
                                            </div>
                                        </div>
                                    ))}
                                </div>
